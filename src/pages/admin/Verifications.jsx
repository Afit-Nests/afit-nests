...
const SIDEBAR_LINKS = [
  ...
  { to: '/admin/verifications', icon: '✅', label: 'Verifications', active: true },
...
export default function Verifications() {
  ...
  const handleApprove = async (id) => {
    ...
    if (!error) {
      setLandlords(landlords.map(l => l.id === id ? { ...l, verified: true } : l))
      setSelected(null)
    }
  }
  ...
  const handleReject = async (id) => {
    ...
    if (!error) {
      setLandlords(landlords.map(l => l.id === id ? { ...l, verified: false } : l))
      setSelected(null)
    }
  }
      ...
      <div className="main-content" style={{ padding: '2.5rem', overflowY: 'auto' }}>
        ...
        <div className="admin-split-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '1.5rem', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
            ...
            {list.map(landlord => (
              <div key={landlord.id} onClick={() => setSelected(landlord)} style={{
                background: 'var(--card)', borderRadius: '16px', padding: '1.2rem 1.4rem',
                border: `2px solid ${selected?.id === landlord.id ? 'var(--orange)' : 'var(--beige-dark)'}`,
                cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', transition: 'all 0.2s',
              }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.1rem' }}>
                      {landlord.phone} · {new Date(landlord.created_at).toLocaleDateString()}
                    </div>
                  </div>
                </div>
                <div style={{
                  fontSize: '0.75rem', fontWeight: 700, padding: '0.25rem 0.8rem', borderRadius: '50px', flexShrink: 0,
                  background: landlord.verified ? 'rgba(22,163,74,0.1)' : 'rgba(249,115,22,0.1)',
                  color: landlord.verified ? '#16A34A' : 'var(--orange)',
                }}>
          ...
          {selected ? (
            <div style={{ background: 'var(--card)', borderRadius: '20px', padding: '1.8rem', border: '1px solid var(--beige-dark)', position: 'sticky', top: '2rem' }}>
              ...
              {[
                { label: 'Phone Number', value: selected.phone, icon: '📞' },
                { label: 'NIN', value: selected.nin || 'Not provided', icon: '🪪' },
                { label: 'Property Address', value: selected.address || 'Not provided', icon: '📍' },
              ].map(field => (
                <div key={field.label} style={{ marginBottom: '1rem', padding: '0.9rem', background: 'var(--beige)', borderRadius: '12px', border: '1px solid var(--beige-dark)' }}>
                  <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)', marginBottom: '0.3rem' }}>
              ...
              {!selected.verified ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', marginTop: '1.5rem' }}>
                  <button onClick={() => handleApprove(selected.id)} style={{ background: '#16A34A', color: 'white', padding: '0.85rem', borderRadius: '50px', fontWeight: 700, fontSize: '0.95rem', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                    ✅ Approve Landlord
                  </button>
                  <button onClick={() => handleReject(selected.id)} style={{ background: 'transparent', color: '#DC2626', padding: '0.85rem', borderRadius: '50px', fontWeight: 700, fontSize: '0.95rem', border: '2px solid #DC2626', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                    ❌ Reject Registration
                  </button>
                </div>
              ) : (
                <div style={{ marginTop: '1.5rem', padding: '1rem', borderRadius: '12px', textAlign: 'center', background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.2)', color: '#16A34A', fontWeight: 600, fontSize: '0.88rem' }}>
                  ✅ This landlord has been verified
                </div>
              )}
            </div>
          ) : (
