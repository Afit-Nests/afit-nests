...
const SIDEBAR_LINKS = [
  ...
  { to: '/student/chats', icon: <MessageSquare size={18} />, label: 'My Chats', active: true },
    ...
    <div className="dashboard-layout" style={{ minHeight: '100vh', background: 'var(--beige)', display: 'grid', gridTemplateColumns: '240px 1fr' }}>
      ...
      <div className="desktop-sidebar" style={{ background: 'var(--blue-dark)', padding: '2rem 1.2rem', display: 'flex', flexDirection: 'column', gap: '0.4rem', position: 'sticky', top: 0, height: '100vh' }}>
        <Link to="/" style={{ textDecoration: 'none', marginBottom: '2rem', display: 'block' }}>
          ...
          <div style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.5)', marginTop: '0.2rem', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Student Panel</div>
        </Link>
      ...
      <div className="chat-layout" style={{ display: 'grid', gridTemplateColumns: '300px 1fr', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ borderRight: '1px solid var(--beige-dark)', overflowY: 'auto', background: 'var(--card)' }}>
          <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--beige-dark)' }}>
            <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '1.2rem', fontWeight: 900, color: 'var(--blue-dark)' }}>My Chats</h2>
          </div>
          {chats.length === 0 ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.78rem', marginTop: '0.3rem' }}>Browse listings and start a conversation</p>
              <Link to="/listings" style={{ display: 'inline-block', marginTop: '1rem', color: 'var(--orange)', fontWeight: 600, fontSize: '0.82rem', textDecoration: 'none' }}>Browse Listings →</Link>
            </div>
          ) : (
            chats.map(chat => (
              <div key={chat.id} onClick={() => setSelectedChat(chat)} style={{ padding: '1.2rem 1.4rem', cursor: 'pointer', background: selectedChat?.id === chat.id ? 'var(--beige)' : 'transparent', borderBottom: '1px solid var(--beige-dark)', borderLeft: `3px solid ${selectedChat?.id === chat.id ? 'var(--orange)' : 'transparent'}` }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.8rem' }}>
                  <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontFamily: 'Playfair Display, serif', flexShrink: 0 }}>
                    {chat.profiles?.full_name?.charAt(0) || 'L'}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 700, fontSize: '0.88rem', color: 'var(--text)', marginBottom: '0.2rem' }}>{chat.profiles?.full_name || 'Landlord'}</div>
        ...
        {selectedChat ? (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
            <div style={{ padding: '1.2rem 1.6rem', background: 'var(--card)', borderBottom: '1px solid var(--beige-dark)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'var(--blue)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontFamily: 'Playfair Display, serif' }}>
                {selectedChat.profiles?.full_name?.charAt(0) || 'L'}
              </div>
              <div>
                <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text)' }}>{selectedChat.profiles?.full_name || 'Landlord'}</div>
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '65%', padding: '0.75rem 1rem', borderRadius: isMe ? '18px 18px 4px 18px' : '18px 18px 18px 4px', background: isMe ? 'var(--orange)' : 'var(--card)', color: isMe ? 'white' : 'var(--text)', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ padding: '1rem 1.6rem', background: 'var(--card)', borderTop: '1px solid var(--beige-dark)', display: 'flex', gap: '0.8rem', alignItems: 'flex-end' }}>
              <textarea
                ...
                rows={1}
                style={{ flex: 1, padding: '0.75rem 1rem', borderRadius: '12px', border: '1px solid var(--beige-dark)', background: 'var(--beige)', fontSize: '0.88rem', color: 'var(--text)', fontFamily: 'DM Sans, sans-serif', resize: 'none', outline: 'none' }}
              />
              <button onClick={handleSend} style={{ background: 'var(--orange)', color: 'white', width: '42px', height: '42px', borderRadius: '50%', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Send size={18} />
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ fontWeight: 600 }}>Select a chat to continue</p>
            </div>
          </div>
        )}
      </div>
