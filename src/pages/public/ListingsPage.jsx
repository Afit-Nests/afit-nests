...
export default function ListingsPage() {
  ...
  const resetFilters = () => { setSelectedType('All'); setMaxPrice(150000); setMaxDistance(60) }
  ...
  const FiltersContent = () => (
    <>
      <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontWeight: 700, fontSize: '1rem', color: 'var(--blue)', marginBottom: '1.5rem' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <label style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text)', display: 'block', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Max Price</label>
        <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--blue)', fontFamily: 'Playfair Display, serif', marginBottom: '0.6rem' }}>₦{maxPrice.toLocaleString()}</div>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <label style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--text)', display: 'block', marginBottom: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Max Distance from AFIT</label>
        <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--blue)', fontFamily: 'Playfair Display, serif', marginBottom: '0.6rem' }}>{maxDistance} mins walk</div>
      ...
      <button onClick={resetFilters} style={{ width: '100%', background: 'transparent', color: 'var(--text-muted)', border: '1px solid var(--beige-dark)', padding: '0.6rem', borderRadius: '50px', fontSize: '0.82rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
        Reset Filters
      </button>
    </>
  )
    ...
    <div>
      ...
      <div style={{ paddingTop: '80px', minHeight: '100vh', background: 'var(--beige)' }}>
        <div style={{ padding: '2rem 5% 1rem' }}>
          <div style={{ textTransform: 'uppercase', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '1.5px', color: 'var(--orange)', marginBottom: '0.4rem' }}>Barkallahu Area</div>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '2rem', fontWeight: 900, color: 'var(--blue-dark)' }}>Available Listings</h1>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '0.3rem', flexWrap: 'wrap', gap: '0.8rem' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>{filtered.length} properties found</p>
            <button className="mobile-filter-btn" onClick={() => setShowFilters(!showFilters)} style={{ display: 'none', alignItems: 'center', gap: '0.4rem', background: 'var(--blue)', color: 'white', border: 'none', padding: '0.5rem 1.2rem', borderRadius: '50px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
        ...
        {showFilters && (
          <div className="mobile-filters-panel" style={{ margin: '0 5% 1rem', background: 'var(--card)', borderRadius: '20px', padding: '1.5rem', border: '1px solid var(--beige-dark)' }}>
            <FiltersContent />
          </div>
        )}
                ...
                {filtered.map(listing => (
                  <div key={listing.id} onClick={() => setSelectedId(listing.id)} style={{ background: 'var(--card)', borderRadius: '20px', overflow: 'hidden', border: `2px solid ${selectedId === listing.id ? 'var(--orange)' : 'var(--beige-dark)'}`, cursor: 'pointer', transition: 'all 0.2s', boxShadow: selectedId === listing.id ? '0 8px 30px rgba(249,115,22,0.2)' : 'none' }}>
                    <div style={{ height: '160px', background: 'linear-gradient(135deg, var(--blue) 0%, #2A5298 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}>
                      ...
                      <div style={{ position: 'absolute', top: '0.8rem', left: '0.8rem', background: 'var(--orange)', color: 'white', fontSize: '0.7rem', fontWeight: 700, padding: '0.25rem 0.7rem', borderRadius: '50px' }}>
                        Available
                      </div>
                    </div>
                    <div style={{ padding: '1.1rem' }}>
                      <div style={{ fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--orange)', marginBottom: '0.3rem' }}>{listing.type}</div>
                      <div style={{ fontWeight: 700, fontSize: '0.9rem', color: 'var(--text)', marginBottom: '0.3rem' }}>{listing.title}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: '0.8rem' }}>
                        <MapPin size={12} /> {listing.distance} mins from AFIT
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <Link to={`/listings/${listing.id}`} onClick={e => e.stopPropagation()} style={{ background: 'var(--blue)', color: 'white', padding: '0.4rem 0.9rem', borderRadius: '50px', fontSize: '0.75rem', fontWeight: 600, textDecoration: 'none' }}>
                          View
                        </Link>
                      </div>
                    </div>
