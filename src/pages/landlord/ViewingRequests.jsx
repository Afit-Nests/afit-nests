...
const SIDEBAR_LINKS = [
  ...
  { to: '/landlord/viewings', icon: '📅', label: 'Viewing Requests', active: true },
              ...
              <div key={viewing.id} style={{ background: 'var(--card)', borderRadius: '20px', padding: '1.8rem', border: '1px solid var(--beige-dark)' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '1rem', alignItems: 'flex-start' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.82rem', color: 'var(--orange)', fontWeight: 600, marginBottom: '0.5rem' }}>🏠 {viewing.listings?.title}</div>
                    <div style={{ display: 'flex', gap: '1rem', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text)', fontWeight: 500, background: 'var(--beige)', padding: '0.3rem 0.8rem', borderRadius: '50px', border: '1px solid var(--beige-dark)' }}>📅 {viewing.date}</div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text)', fontWeight: 500, background: 'var(--beige)', padding: '0.3rem 0.8rem', borderRadius: '50px', border: '1px solid var(--beige-dark)' }}>🕐 {viewing.time}</div>
                    </div>
                    {viewing.message && (
                    ...
                    {viewing.status === 'confirmed' && viewing.profiles?.phone && (
                      <div style={{ marginTop: '0.8rem', background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.2)', borderRadius: '12px', padding: '0.7rem 1rem', fontSize: '0.85rem', color: '#16A34A', fontWeight: 600 }}>
                        📞 Student contact: {viewing.profiles.phone}
                      </div>
                    )}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.8rem' }}>
                    <div style={{ padding: '0.35rem 0.9rem', borderRadius: '50px', fontSize: '0.75rem', fontWeight: 700, background: style.bg, border: `1px solid ${style.border}`, color: style.color }}>{style.label}</div>
                    {viewing.status === 'pending' && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <button onClick={() => handleConfirm(viewing.id)} style={{ background: '#16A34A', color: 'white', padding: '0.5rem 1.2rem', borderRadius: '50px', border: 'none', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>✅ Confirm</button>
                        <button onClick={() => handleDecline(viewing.id)} style={{ background: 'transparent', color: '#DC2626', padding: '0.5rem 1.2rem', borderRadius: '50px', border: '2px solid #DC2626', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>❌ Decline</button>
                      </div>
                    )}
                  </div>
                </div>
