...
const SIDEBAR_LINKS = [
  ...
  { to: '/admin/disputes', icon: <AlertTriangle size={18} />, label: 'Disputes', active: true },
              ...
              <div key={dispute.id} onClick={() => handleSelect(dispute)} style={{ background: 'var(--card)', borderRadius: '16px', padding: '1.4rem', border: `2px solid ${selected?.id === dispute.id ? 'var(--orange)' : 'var(--beige-dark)'}`, cursor: 'pointer', transition: 'border-color var(--duration-panel) ease' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.8rem' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>{dispute.listings?.title} · {new Date(dispute.created_at).toLocaleDateString()}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', fontWeight: 700, padding: '0.25rem 0.8rem', borderRadius: '50px', flexShrink: 0, background: dispute.status === 'open' ? 'rgba(249,115,22,0.1)' : 'rgba(22,163,74,0.1)', color: dispute.status === 'open' ? 'var(--orange)' : '#16A34A' }}>
                    {dispute.status === 'open' ? <AlertTriangle size={12} /> : <CheckCircle size={12} />}
                    {dispute.status === 'open' ? 'Open' : 'Resolved'}
                  </div>
                </div>
                <p style={{ fontSize: '0.83rem', color: 'var(--text-muted)', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{dispute.issue}</p>
              </div>
            ))}
          </div>
          ...
          {selected ? (
            <div style={{ background: 'var(--card)', borderRadius: '20px', border: '1px solid var(--beige-dark)', overflow: 'hidden', position: 'sticky', top: '2rem' }}>
              <div style={{ padding: '1.4rem 1.6rem', borderBottom: '1px solid var(--beige-dark)' }}>
                <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--text)', marginBottom: '0.2rem' }}>{selected.profiles?.full_name} vs {selected.landlord?.full_name}</div>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>{selected.listings?.title} · {selected.profiles?.matric_number}</div>
              </div>

              <div style={{ padding: '1.2rem 1.6rem', background: 'rgba(249,115,22,0.05)', borderBottom: '1px solid var(--beige-dark)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--orange)', marginBottom: '0.4rem' }}>
              ...
              <div style={{ padding: '1.2rem 1.6rem', maxHeight: '240px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.8rem', borderBottom: '1px solid var(--beige-dark)' }}>
                ...
                {(selected.messages || []).map((msg, i) => (
                  <div key={i} style={{ padding: '0.8rem', borderRadius: '12px', background: msg.sender_role === 'admin' ? 'rgba(27,58,107,0.08)' : 'var(--beige)', border: `1px solid ${msg.sender_role === 'admin' ? 'rgba(27,58,107,0.15)' : 'var(--beige-dark)'}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.3rem' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', fontWeight: 700, color: msg.sender_role === 'admin' ? 'var(--blue)' : 'var(--text-muted)' }}>
                        {msg.sender_role === 'admin' ? <Shield size={12} /> : null}
                        {msg.sender_role === 'admin' ? 'Admin' : msg.sender_role}
                      </span>
                      <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
              ...
              <div style={{ padding: '1.2rem 1.6rem', borderBottom: '1px solid var(--beige-dark)' }}>
                ...
                <button onClick={handleSendMessage} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginTop: '0.6rem', width: '100%', background: 'var(--blue)', color: 'white', padding: '0.75rem', borderRadius: '50px', fontWeight: 600, fontSize: '0.88rem', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
              ...
              {selected.status === 'open' && (
                <div style={{ padding: '1.2rem 1.6rem' }}>
                  <button onClick={() => handleResolve(selected.id)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', width: '100%', background: '#16A34A', color: 'white', padding: '0.75rem', borderRadius: '50px', fontWeight: 600, fontSize: '0.88rem', border: 'none', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
